// Trimmed copy of RN 0.83.1 ReactCommon/.../RCTTurboModule.h.
// LAYOUT-CRITICAL: ObjCTurboModule and its nested struct InitParams must match
// the binary exactly (the React dylib constructs ObjCInteropTurboModule(InitParams)).
// Deviations: dropped <React/RCTBridge.h>/<React/RCTModuleMethod.h> (only used by
// the removed RCTBridge category) and the vendored "RCTBridgeModule.h" satisfies
// the RCTBridgeModule import.
...
// Original's TurboModuleConvertUtils + Bridging<id> specialization dropped:
// not part of ObjCTurboModule's layout and unused here.
